...
function getHTTPHeaders($options=''){
  // setting the options allows the adjustment of the headers
  // values at the moment are:
  // 'Content-Disposition'=>'filename'  - sets the filename, though not too sure how well this will 
  //        work as in my trial the browser seems to use the filename of the php file with .pdf on the end
  ...
  $headers[] = "Content-Length: ".strlen(ltrim($tmp));
  if(isset($options['Content-Disposition']))
  {
    $headers[] = "Content-Disposition: attachment; filename=".$options['Content-Disposition'];
  }
  else
  {
    ...
    if ($directive){
      ...
      if ($w>$tw){
        ...
        if ($break>0){
          ...
          //echo "<p>ERROR @ ".__LINE__.": '".substr($text,$break+1)."'</p>";
          ...
          //echo "<p>ERROR @ ".__LINE__.": '".substr($text,$i)."'</p>";
